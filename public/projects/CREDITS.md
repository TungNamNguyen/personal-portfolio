...
| `sparkev.webp` | Own work — the Performance dashboard, rendered from Tableau Public so the cover matches where the card's link goes | — |
| `databel.webp` | Own work — the Churn Analysis dashboard, rendered from Tableau Public | — |
| `refugees.webp` | Own work — the choropleth on the deployed site's main page, captured headless. The Sankey was tried first but it is one view of several, not the headline one | — |
